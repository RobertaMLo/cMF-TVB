...
def init(parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
    ...
    # =================== CRBL_MF ===========================
    # =======================================================
    if parameter_model['robi']:
        import tvb_model_reference.src.parallel_crbl_multimf_ww as model
        ...
        if parameter_model['order'] == 1:
            ...
            model.g_L_grc = np.array(parameter_model['g_L_grc'])
            model.g_L_goc = np.array(parameter_model['g_L_goc'])
            model.g_L_mli = np.array(parameter_model['g_L_mli'])
            model.g_L_pc = np.array(parameter_model['g_L_pc'])

            model.E_L_grc = np.array(parameter_model['E_L_grc'])
            model.E_L_goc = np.array(parameter_model['E_L_goc'])
            model.E_L_mli = np.array(parameter_model['E_L_mli'])
            model.E_L_pc = np.array(parameter_model['E_L_pc'])

            model.C_m_L_grc = np.array(parameter_model['C_m_grc'])
            model.C_m_goc = np.array(parameter_model['C_m_goc'])
            model.C_m_mli = np.array(parameter_model['C_m_mli'])
            model.C_m_pc = np.array(parameter_model['C_m_pc'])

            model.E_e = np.array(parameter_model['E_e'])
            model.E_i = np.array(parameter_model['E_i'])

            model.Q_mf_grc = np.array(parameter_model['Q_mf_grc'])
            model.Q_mf_goc = np.array(parameter_model['Q_mf_goc'])
            model.Q_grc_goc = np.array(parameter_model['Q_grc_goc'])
            model.Q_grc_mli = np.array(parameter_model['Q_grc_mli'])
            model.Q_grc_pc = np.array(parameter_model['Q_grc_pc'])
            model.Q_goc_goc = np.array(parameter_model['Q_goc_goc'])
            model.Q_goc_grc = np.array(parameter_model['Q_goc_grc'])
            model.Q_mli_mli = np.array(parameter_model['Q_mli_mli'])
            model.Q_mli_pc = np.array(parameter_model['Q_mli_pc'])

            model.tau_mf_grc = np.array(parameter_model['tau_mf_grc'])
            model.tau_mf_goc = np.array(parameter_model['tau_mf_goc'])
            model.tau_grc_goc = np.array(parameter_model['tau_grc_goc'])
            model.tau_grc_mli = np.array(parameter_model['tau_grc_mli'])
            model.tau_grc_pc = np.array(parameter_model['tau_grc_pc'])
            model.tau_goc_goc = np.array(parameter_model['tau_goc_goc'])
            model.tau_goc_grc = np.array(parameter_model['tau_goc_grc'])
            model.tau_mli_mli = np.array(parameter_model['tau_mli_mli'])
            model.tau_mli_pc = np.array(parameter_model['tau_mli_pc'])

            model.N_tot_grc = np.array(parameter_model['N_tot_grc'])
            model.N_tot_goc = np.array(parameter_model['N_tot_goc'])
            model.N_tot_mli = np.array(parameter_model['N_tot_mli'])
            model.N_tot_pc = np.array(parameter_model['N_tot_pc'])
            model.N_tot_mossy = np.array(parameter_model['N_tot_mossy'])

            model.K_mf_grc = np.array(parameter_model['K_mf_grc'])
            model.K_mf_goc = np.array(parameter_model['K_mf_goc'])
            model.K_grc_goc = np.array(parameter_model['K_grc_goc'])
            model.K_grc_mli = np.array(parameter_model['K_grc_mli'])
            model.K_grc_pc = np.array(parameter_model['K_grc_pc'])
            model.K_goc_goc = np.array(parameter_model['K_goc_goc'])
            model.K_goc_grc = np.array(parameter_model['K_goc_grc'])
            model.K_mli_mli = np.array(parameter_model['K_mli_mli'])
            model.K_mli_pc = np.array(parameter_model['K_mli_pc'])

            model.T = np.array(parameter_model['T'])

            model.P_GrC_e = np.array(parameter_model['P_GrC_e'])
            model.P_GoC_i = np.array(parameter_model['P_GoC_i'])
            model.P_MLI_i = np.array(parameter_model['P_MLI_i'])
            model.P_PC_i = np.array(parameter_model['P_PC_i'])


            model.a_e = np.array(parameter_model['a_e'])
            model.b_e = np.array(parameter_model['b_e'])
            model.d_e = np.array(parameter_model['d_e'])
            model.gamma_e = np.array(parameter_model['gamma_e'])
            model.tau_e = np.array(parameter_model['tau_e'])
            model.w_p = np.array(parameter_model['w_p'])
            model.J_N = np.array(parameter_model['J_N'])
            model.W_e = np.array(parameter_model['W_e'])

            model.a_i = np.array(parameter_model['a_i'])
            model.b_i = np.array(parameter_model['b_i'])
            model.d_i = np.array(parameter_model['d_i'])
            model.gamma_i = np.array(parameter_model['gamma_i'])
            model.tau_i = np.array(parameter_model['tau_i'])
            model.W_i = np.array(parameter_model['W_i'])
            
            model.I_o = np.array(parameter_model['I_o'])
            model.I_ext = np.array(parameter_model['I_ext'])
            model.G = np.array(parameter_model['G'])
            model.lamda = np.array(parameter_model['lambda'])
            model.I_ext_ampl = np.array(parameter_model['I_ext_ampl'])
            ...
            model.state_variable_range['d1'] = np.array(parameter_model['initial_condition']['d1'])
            model.state_variable_range['d2'] = np.array(parameter_model['initial_condition']['d2'])
            model.state_variable_range['d3'] = np.array(parameter_model['initial_condition']['d3'])
            model.state_variable_range['d4'] = np.array(parameter_model['initial_condition']['d4'])

        else:
            raise Exception('Bad order for the model. Sorry, order 2 is still work in progress!')

    else:
        import tvb_model_references.src.crbl_mf as model
...
def run_sim_for_bold(simulator, time):
    '''
    run a simulation
    :param simulator: the simulator already initialize
    :param time: the time of simulation
    ...
    returns: 
    tavg time, tavg data (squeezed ready to be used for pcc), 
    bold time, bold data (squeezed)
    '''
    ...
    for tavg, bold in simulator(simulation_length=time):  
        ...
        if c_it % 2000 ==0:
            print('ITERATION NUMBER: ',c_it)
            print('**********************************************************************************')

        c_it = c_it + 1
        

    tavg_data_squeezed = np.squeeze(tavg_data)
    bold_data_squeezed = np.squeeze(bold_data)

    return tavg_time, tavg_data_squeezed, bold_time, bold_data_squeezed


def run_sim_only_bold(simulator, time):
    '''
    run a simulation
    :param simulator: the simulator already initialize
    :param time: the time of simulation
    ...
    for bold in simulator(simulation_length=time):  

        if not bold is None:
            bold_time.append(bold[0])
            bold_data.append(bold[1])

    return bold_time, bold_data
...
def print_bistability(parameter_model,show=True):
    ...
    ## Model
    if parameter_model['order'] == 1:
        model = model.crbl_mf_first_order(variables_of_interest='E I W_e W_i'.split())
