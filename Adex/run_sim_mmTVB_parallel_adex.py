...
from tools_for_sim_mmTVB_parallel_adex import *
import argparse
import numpy as np
import os
...
def main():
    ...
    #output_folder = os.path.join(SUB_DIR,'TVB_output_MFTVB/')

    output_folder = os.path.join(SUB_DIR,'TVB_output_MFTVB', id_sim)
    os.makedirs(output_folder, exist_ok=True)

    id_cortex = 20 #set the final index for saving timeseries (avoid to have super long file)
    id_crbl_first = 93
    id_crbl_last = 126 #same as above
    ...
    simname = date_time + '_'+ id_sim +'_ctx'+str(id_cortex)+'_crbl'+str(id_crbl_last)+'_sim'+str(int(sim_len*1e-3))
    ...
    # Saving output -------------------------------------------------------------------------------------------------------------------
    saving_TVB_output(tavgtime, tavgdata, boldtime, bolddata, output_folder, simname, id_cortex, id_crbl_last, id_crbl_first, bool_to_save=True)
